classdef Grid2DUI < handle
    ...
    methods (Access=private)
        ...
        function updateProj(obj,varargin)
            ...
            obj.grid.Tx = transl_rotat(obj.data.Tx_p, obj.grid.x0, az, dip);
            obj.grid.Rx = transl_rotat(obj.data.Rx_p, obj.grid.x0, az, dip);
            obj.grid.TxCosDir = transl_rotat(obj.data.TxCosDir, [0 0 0], az, dip);
            obj.grid.RxCosDir = transl_rotat(obj.data.RxCosDir, [0 0 0], az, dip);
            if ~isnan(obj.data.Tx_Z_water(1,1))
                obj.grid.Tx_Z_water = transl_rotat(obj.data.Tx_Z_water, origine, az, dip);
            end
            if ~isnan(obj.data.Rx_Z_water(1,1))
                obj.grid.Rx_Z_water = transl_rotat(obj.data.Rx_Z_water, origine, az, dip);
            end
