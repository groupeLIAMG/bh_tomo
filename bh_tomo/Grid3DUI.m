classdef Grid3DUI < handle
    ...
    methods (Access=private)
        function addComponents(obj,f,fs)
            ...
            bhNames = cell(1+length(obj.data.boreholes),1);
            bhNames{1} = '-';
            for n=1:length(obj.data.boreholes)
                bhNames{n+1} = obj.data.boreholes(n).name;
            end
            obj.handles.bhList = uicontrol('Style','popupmenu',...
                'String',bhNames,...
                'Value',obj.grid.borehole_x0+1,...
                'FontSize',fs,...
                'Units','points',...
                'Callback',@obj.inputEdit,...
                'Parent',obj.handles.params);
            ...
            obj.handles.rotation = uicontrol('Style','text',...
                'String','Rotation angle (°)',...
                'FontSize',fs,...
                'Units','points',...
                'HorizontalAlignment','right',...
                'Parent',obj.handles.params);
            obj.handles.rot_angle = uicontrol('Style','edit',...
                'String',num2str(obj.grid.rotation),...
                'FontSize',fs,...
                'Units','points',...
                'Callback',@obj.inputEdit,...
                'Parent',obj.handles.params);
        ...
        function resizeUI(obj,varargin)
            ...
            obj.handles.bh_x0.Position = [hBorder height-vBorderTop-4*vSize-3*vSpace 2*hSize vSize];
            obj.handles.bhList.Position = [hBorder+hSpace+2*hSize height-vBorderTop-4*vSize-3*vSpace 2*hSize vSize];

            obj.handles.origin.Position = [hBorder height-vBorderTop-5*vSize-4*vSpace hSize vSize];
            obj.handles.x0x.Position = [hBorder+hSize+hSpace height-vBorderTop-5*vSize-4*vSpace hSize vSize];
            obj.handles.x0y.Position = [hBorder+2*hSize+2*hSpace height-vBorderTop-5*vSize-4*vSpace hSize vSize];
            obj.handles.x0z.Position = [hBorder+3*hSize+3*hSpace height-vBorderTop-5*vSize-4*vSpace hSize vSize];

            obj.handles.rotation.Position = [hBorder height-vBorderTop-6*vSize-5*vSpace 2*hSize vSize];
            obj.handles.rot_angle.Position = [hBorder+2*hSize+2*hSpace height-vBorderTop-6*vSize-5*vSpace hSize vSize];
            
            obj.handles.numCores.Position = [2*hBorder height-vBorderTop-7.5*vSize-6*vSpace 2*hSize vSize];
            obj.handles.ncoreList.Position = [3*hBorder+2*hSize height-vBorderTop-7.5*vSize-6*vSpace hSize vSize];
        ...
        function inputEdit(obj,varargin)
            ...
            obj.grid.rotation = deg2rad(-str2double( obj.handles.rot_angle.String ));
            obj.grid.borehole_x0 = obj.handles.bhList.Value-1;
            if obj.grid.borehole_x0 > 0
        ...
        function updateGrid(obj)
            ...
            az = obj.grid.rotation;
            obj.grid.Tx = Grid.transl_rotat(obj.data.Tx, obj.grid.x0, az, 0);
            obj.grid.Rx = Grid.transl_rotat(obj.data.Rx, obj.grid.x0, az, 0);
            obj.grid.TxCosDir = Grid.transl_rotat(obj.data.TxCosDir, [0 0 0], az, 0);
            obj.grid.RxCosDir = Grid.transl_rotat(obj.data.RxCosDir, [0 0 0], az, 0);
            if ~isnan(obj.data.Tx_Z_water(1,1))
                obj.grid.Tx_Z_water = Grid.transl_rotat(obj.data.Tx_Z_water, obj.grid.x0, az, 0);
            end
            if ~isnan(obj.data.Rx_Z_water(1,1))
                obj.grid.Rx_Z_water = Grid.transl_rotat(obj.data.Rx_Z_water, obj.grid.x0, az, 0);
            end
