...
uicontrol('Style','pushbutton',...
    ...
    'FontWeight','bold',...
    'Units','normalized',...
    'Position',[0.3 0.5*vSpace 0.4 vSize],...
    'Callback',@doInv,...
    'Parent',pinv);

psimult = uipanel(pinv,'Title','Parameters - Simultaneous Inversion',...
    'Units','normalized',...
    'Position',[0.025 1*vSpace+1.25*vSize 0.95 9*vSize+9*vSpace],...
    'FontSize',fs,...
    'Visible','on');
pdiff = uipanel(pinv,'Title','Parameters - Difference Inversion',...
    ...
    'FontSize',fs,...
    'Visible','off');
pdelta = uipanel(pinv,'Title',['Parameters - ',char(916),'t / ',char(916),char(964),' Inversion'],...
    'Units','normalized',...
    'Position',[0.025 1*vSpace+1.25*vSize 0.95 9*vSize+9*vSpace],...
    'FontSize',fs,...
    'Visible','off');
...
pweight = uipanel(pinv,'Title','Weight - Reservoir Cells',...
    'Units','normalized',...
    'Position',[0.025 12*vSpace+10.7*vSize 0.95 1.5*vSize+2*vSpace],...
    'FontSize',fs,...
    'Visible','on');
pref = uipanel(pinv,'Title','Reference Tomogram',...
    'Units','normalized',...
    'Position',[0.025 14*vSpace+12.75*vSize 0.95 1.5*vSize+2*vSpace],...
    'FontSize',fs,...
    'Visible','on');
...
uicontrol('Style','text',...
    ...
    'Position',[0.1 17*vSpace+15.75*vSize 0.3 vSize],...
    'Parent',pinv);
htypeInv = uicontrol('Style','popupmenu',...
    'String',{'Simultaneous Inversion',...
    'Difference Inversion',...
    [char(916),'t / ',char(916),char(964),' Inversion']},...
    'FontSize',fs,...
    'Units','normalized',...
    'Position',[0.45 17*vSpace+15.75*vSize 0.4 vSize],...
...
hnumIt = uicontrol('Style','edit',...
    ...
    'FontSize',fs,...
    'Units','normalized',...
    'Position',[0.45 16*vSpace+14.75*vSize 0.2 vSize],...
    'Callback',@changeTypeInv,...
    'Parent',pinv);
...
uicontrol('Style','text',...
    ...
    'HorizontalAlignment','right',...
    'Units','normalized',...
    'Position',[0.05 9*vSpace+8*vSize 0.4 vSize],...
    'Parent',psimult);
halphaS = uicontrol('Style','edit',...
    'String','1',...
    'FontSize',fs,...
    'Units','normalized',...
    'Position',[0.5 9*vSpace+8*vSize 0.2 vSize],...
    'Parent',psimult);
uicontrol('Style','text',...
    'String',[char(946),' : '],...
    'FontSize',fs,...
...
hdampS = uicontrol('Style','edit',...
    ...
    'Parent',psimult);
uicontrol('Style','text',...
    'String','Smoothing op. order : ',...
    'FontSize',fs,...
    'HorizontalAlignment','right',...
    'Units','normalized',...
    'Position',[0.05 2*vSpace+vSize 0.4 vSize],...
    'Parent',psimult);
horderS = uicontrol('Style','popupmenu',...
    'String',{'1','2'},...
    'FontSize',fs,...
    'Units','normalized',...
...
uicontrol('Style','text',...
    ...
    'Position',[0.05 3*vSpace+2*vSize 0.4 vSize],...
    'Parent',pdiff);
horderD = uicontrol('Style','popupmenu',...
    'String',{'1','2'},...
    'FontSize',fs,...
    'Units','normalized',...
    'Position',[0.5 3*vSpace+2*vSize 0.2 vSize],...
...
uicontrol('Style','text',...
    ...
    'Position',[0.05 4*vSpace+3*vSize 0.4 vSize],...
    'Parent',pdelta);
horderT = uicontrol('Style','popupmenu',...
    'String',{'1','2'},...
    'FontSize',fs,...
    'Units','normalized',...
    'Position',[0.5 4*vSpace+3*vSize 0.2 vSize],...
...
f.Visible = 'on';
    ...
    function openFile(varargin)
        ...
        tlinv_name = cell(1,length(model.tlinv_res));
        for n=1:length(model.tlinv_res)
            tlinv_name{n} = model.tlinv_res(n).name;
        end
        hraysS.String = tlinv_name;
        hraysD.String = tlinv_name;
    ...
    function doInv(varargin)
        ...
        if htypeInv.Value == 1
            ...
            if param.mog_no0 == param.mog_no1
                errordlg('Repeat dataset should not be baseline dataset')
            end
            ...
            param.Dorder = horderS.Value;
            
            param.L_tomo_no = hraysS.Value;
            
            if param.tomoAtt == 1 && ~isfield(model.tlinv_res(param.L_tomo_no).tomo,'no_trace0')
        ...
        elseif htypeInv.Value == 2
            ...
            param.beta = str2double(hbetaT.String);
            param.mux = str2double(hmuxT.String);
            param.muy = str2double(hmuyT.String);
            param.muz = str2double(hmuzT.String);
            param.damping = str2double(hdampT.String);
            param.Dorder = horderT.Value;
            ...
            tomo = invDelta(model,param,hmessage,gh,gridViewer);
        end
        saved = false;
        hmessage.String = '';
