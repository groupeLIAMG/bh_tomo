function varargout = bh_tomo_grille(varargin)
% BH_TOMO_GRILLE M-file for bh_tomo_grille.fig
%      BH_TOMO_GRILLE, by itself, creates a new BH_TOMO_GRILLE or raises the existing
%      singleton*.
%
%      H = BH_TOMO_GRILLE returns the handle to a new BH_TOMO_GRILLE or the handle to
%      the existing singleton*.
%
%      BH_TOMO_GRILLE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BH_TOMO_GRILLE.M with the given input arguments.
%
%      BH_TOMO_GRILLE('Property','Value',...) creates a new BH_TOMO_GRILLE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before bh_tomo_grille_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to bh_tomo_grille_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help bh_tomo_grille

% Last Modified by GUIDE v2.5 18-Oct-2012 10:09:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @bh_tomo_grille_OpeningFcn, ...
                   'gui_OutputFcn',  @bh_tomo_grille_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before bh_tomo_grille is made visible.
function bh_tomo_grille_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to bh_tomo_grille (see VARARGIN)

% Choose default command line output for bh_tomo_grille
...
%         plot_proj(handles)
%     end
    update_proj(handles)  % we force update in case data was added
    update_info(handles)
end

% UIWAIT makes bh_tomo_grille wait for user response (see UIRESUME)
uiwait(handles.fig_bh_grille);

% --- Outputs from this function are returned to the command line.
function varargout = bh_tomo_grille_OutputFcn(hObject, eventdata, handles) 
...
set(handles.pushbutton_qualite,       'String', str.s297)
...
set(handles.checkbox_flip,            'String', str.s272)
...
g.Tx = transl_rotat(h.Tx, origine, az, dip);
g.Rx = transl_rotat(h.Rx, origine, az, dip);
...
plot(g.Rx(g.in,1),g.Rx(g.in,3),'gx')
...
guidata(hObject, handles);
uiresume(handles.fig_bh_grille);
...
g = getappdata(handles.fig_bh_grille,'g');
g.flip = get(hObject,'Value');
setappdata(handles.fig_bh_grille,'g',g)
update_proj(handles)


%%%%%%added 2012-10-18  YH
function fig_bh_grille_CloseRequestFcn(hObject, eventdata, handles)
guidata(hObject, handles);
uiresume(handles.fig_bh_grille);
%delete(hObject);
