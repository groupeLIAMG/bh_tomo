...
% on assume que les forages sont quasi parallèles, i.e. que l'angle
% d'émission à Tx est à peu près égal à l'angle d'incidence à Rx

% on calcule l'angle du rai (droit) p/r à l'horizontale
...
if isempty(h.fdom)
	...
	nt = size(traces,2);
	h.fdom = zeros(1,nt)-1;
    hwb = waitbar(0,'Calcul des fr�quences dominantes');
	inc_wb = 100;
    for n=1:nt
        trace = traces(:,n);
