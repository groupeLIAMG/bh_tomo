...
d.rdata = lisDT1( basename, d.ntrace );
if d.rdata==-1
  return
end
...
function data = lisDT1( basename, nt )
...
if fid < 2
  ...
  if fid < 2
	...
	[fid,mesg]=fopen(fichier,'rt');
	if fid < 2
	  disp(['Impossible d''ouvrir le fichier DT1: ',basename,'\n',mesg])
	  data=-1;
	  return
	end
  end
...
for n=1:nt
	...
	comment = fread(fid, 28, 'char','ieee-le')';
	data = [data fread(fid, header(3), 'int16','ieee-le')];
end
fclose(fid);
