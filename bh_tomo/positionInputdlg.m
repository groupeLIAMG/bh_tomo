function button = positionInputdlg(mogData)

if ~isa(mogData,'MogData')
    error('positionInputdlg: input should be an instance of MogData')
end

button = 'cancel';

width = 445;
height = 250;
% get screen size
su = get(groot,'Units');
set(groot,'Units','points')
scnsize = get(groot,'ScreenSize');
pos = [scnsize(3)/2-width/2 scnsize(4)/2-height/2 width height];
set(groot,'Units',su)       % Restore default root screen units

zstart = 0;
dz = 0.2;
zend = zstart + dz*(mogData.ntrace-1);

f = figure('Visible','off',...
    'Units','points',...
    'Position',pos,...
    'Tag','positionInputdlg',...
    'Name','Antenna Position',...
    'NumberTitle','off',...
    'Resize','off',...
    'ToolBar','none',...,
    'MenuBar','None',...
    'CloseRequestFcn',@closeWindow);

fs = 11;
if ispc
    fs = 9;
end

uicontrol('Style','pushbutton',...
    'String','Cancel',...
    'Units','points',...
    'Position',[15 15 200 22],...
    'Callback',@cancel,...
    'FontSize',fs,...
    'Parent',f);
uicontrol('Style','pushbutton',...
    'String','Done',...
    'Units','points',...
    'Position',[230 15 200 22],...
    'Callback',@done,...
    'FontSize',fs,...
    'Parent',f);

hp1 = uipanel(f,'Title','Fixed Antenna',...
    'Units','points',...
    'Position',[15 45 200 175],...
    'FontSize',fs);

uicontrol('Style','text',...
    'String','Position',...
    'Units','points',...
    'HorizontalAlignment','right',...
    'Position',[15 120 100 22],...
    'FontSize',fs,...
    'Parent',hp1);
hfixedPos = uicontrol('Style','edit',...
    'String','',...
    'Units','points',...
    'Position',[120 120 60 22],...
    'FontSize',fs,...
    'TooltipString','Enter distance along borehole',...
    'Parent',hp1);
    
hfixedType = uicontrol('Style','popupmenu',...
    'String',{'Transmitter','Receiver'},...
    'Units','points',...
    'Position',[25 60 150 22],...
    'FontSize',fs,...
    'Parent',hp1);
    

hp2 = uipanel(f,'Title','Moving Antenna',...
    'Units','points',...
    'Position',[230 45 200 175],...
    'FontSize',fs);

uicontrol('Style','text',...
    'String','Start Position',...
    'Units','points',...
    'HorizontalAlignment','right',...
    'Position',[15 120 100 22],...
    'FontSize',fs,...
    'Parent',hp2);
hmovingPos = uicontrol('Style','edit',...
    'String',sprintf('%g', zstart),...
    'Units','points',...
    'Position',[120 120 60 22],...
    'FontSize',fs,...
    'Callback',@updateEnd,...
    'TooltipString','Enter distance along borehole',...
    'Parent',hp2);

uicontrol('Style','text',...
    'String','Increment',...
    'Units','points',...
    'HorizontalAlignment','right',...
    'Position',[15 85 100 22],...
    'FontSize',fs,...
    'Parent',hp2);
hinc = uicontrol('Style','edit',...
    'String',sprintf('%g', dz),...
    'Units','points',...
    'Position',[120 85 60 22],...
    'FontSize',fs,...
    'Callback',@updateEnd,...
    'TooltipString','Distance Between measurements',...
    'Parent',hp2);

uicontrol('Style','text',...
    'String','End Position',...
    'Units','points',...
    'HorizontalAlignment','right',...
    'Position',[15 50 100 22],...
    'FontSize',fs,...
    'Parent',hp2);

hend = uicontrol('Style','text',...
    'String',sprintf('%g', zend),...
    'Units','points',...
    'HorizontalAlignment','left',...
    'Position',[120 50 100 22],...
    'FontSize',fs,...
    'Parent',hp2);

hupdown = uicontrol('Style','popupmenu',...
    'String',{'Downward','Upward'},...
    'Units','points',...
    'Position',[25 15 150 22],...
    'FontSize',fs,...
    'Callback',@updateEnd,...
    'Parent',hp2);

uiwait(f)

    function closeWindow(varargin)
        delete(f)
    end
    function cancel(varargin)
        button = 'cancel';
        uiresume(f);
        closeWindow()
    end
    function updateEnd(varargin)
        if hupdown.Value == 1
            sign=1;  % positive downward
        else
            sign=-1;
        end
        dz=str2double(hinc.String);
        zend = str2double(hmovingPos.String)+sign*dz*(mogData.ntrace-1);
        hend.String = sprintf('%g', zend);
    end
    function done(varargin)
        
        if isempty(hfixedPos.String) || isempty(hmovingPos.String) || ...
                isempty(hinc.String)
            warndlg('Enter numeric values')
            return
        end
        
        zFixed = str2double(hfixedPos.String)+zeros(1,mogData.ntrace);
        if hupdown.Value == 1
            sign=1;  % positive downward
        else
            sign=-1;
        end
        dz=str2double(hinc.String);
        zMobile = str2double(hmovingPos.String)+sign*dz*(0:(mogData.ntrace-1));
        
        if hfixedType.Value==1
            mogData.Tx_z = zFixed;
            mogData.Rx_z = zMobile;
        else
            mogData.Tx_z = zMobile;
            mogData.Rx_z = zFixed;
        end
        
        button = 'ok';
        uiresume(f);
        closeWindow()
    end

end
