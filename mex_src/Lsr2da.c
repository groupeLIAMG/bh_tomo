/*
 * Matrix L from straight rays, in 2D elliptical anisotropic media
 *
 * Lsr2da
 *
 *  Created by Bernard Giroux on 09-12-18.
...
{
  double        dx, dz, xs, zs, xr, zr, dlx, dlz, x, z, m, b, xe, ze;
  double        percent_sp, dtmp, zi;
  const double  small=1.e-10;
  double        *Tx, *Rx, *grx, *grz, *Lval, *gridx, *gridz, *Lvec;
  mwSize        nTx, nRx, n_grx, n_grz, nSlowness, nCells;
  ...
  if (nrhs != 4) {
		mexErrMsgTxt("LSR2DA requires four input arguments.");
  } else if (nlhs > 3) {
		mexErrMsgTxt("LSR2DA has a maximum of three output argument.");
  }
