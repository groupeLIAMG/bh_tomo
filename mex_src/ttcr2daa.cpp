 ...
 * ttcr2daa
 *
 *  Created by Bernard Giroux on 10-05-03.
 *  Copyright 2010 Bernard Giroux.
 *
 * Reference paper
 *
 * @article{gruber:1062,
...
{
  double        *slowness, *xi, *theta;
	double        *xmin, *zmin, *dx, *dz, *nx_d, *nz_d, *nsx_d, *nsz_d;
  double        *Tx, *Rx, *t_arr, *Lval, *rays_p;
  size_t        nSlowness, nXi, nTheta, stmp, nTx, nRx, ns, nCells;
	int           rValue;
  ...
  if (nrhs != 6) {
		mexErrMsgTxt("TTCR2DAA requires six input arguments.");
  } else if (nlhs > 3) {
		mexErrMsgTxt("TTCR2DAA has a maximum of three output argument.");
  }
  ...
  if (!(mxIsDouble(prhs[2]))) {
		mexErrMsgTxt("Anisotropy angle must be double precision.");
  }
  theta = static_cast<double*>( mxGetPr(prhs[2]) );
  number_of_dims = mxGetNumberOfDimensions(prhs[2]);
  if ( number_of_dims != 2 ) {
		mexErrMsgTxt("Anisotropy angle must be a vector (nTheta by 1).");
  }
  dim_array = mxGetDimensions(prhs[2]);
  nTheta = static_cast<size_t>( dim_array[0] );
  stmp = static_cast<size_t>( dim_array[1] );
  if ( stmp != 1 ) {
		mexErrMsgTxt("Anisotropy angle must be a vector (nXi by 1).");
  }
  ...
  xmin  = static_cast<double*>( mxGetPr( mxGetField(prhs[3], 0, "xmin") ) );
  zmin  = static_cast<double*>( mxGetPr( mxGetField(prhs[3], 0, "zmin") ) );
  dx    = static_cast<double*>( mxGetPr( mxGetField(prhs[3], 0, "dx") ) );
  dz    = static_cast<double*>( mxGetPr( mxGetField(prhs[3], 0, "dz") ) );
  nx_d  = static_cast<double*>( mxGetPr( mxGetField(prhs[3], 0, "nx") ) );
  nz_d  = static_cast<double*>( mxGetPr( mxGetField(prhs[3], 0, "nz") ) );
  nsx_d = static_cast<double*>( mxGetPr( mxGetField(prhs[3], 0, "nsx") ) );
  nsz_d = static_cast<double*>( mxGetPr( mxGetField(prhs[3], 0, "nsz") ) );
  ...
  if ( nXi != nCells ) {
		mexErrMsgTxt("Anisotropy ratio size inconsistent with grid params.");
  }
  if ( nTheta != nCells ) {
		mexErrMsgTxt("Anisotropy angle size inconsistent with grid params.");
  }
  ...
  Tx = static_cast<double*>( mxGetPr(prhs[4]) );
  ...
  Rx = static_cast<double*>( mxGetPr(prhs[5]) );
  ...
  /* ------------------------------------------------------
	...
	Grid2Daa<double,uint32_t> grid(gp.nx, gp.nz, gp.dx, gp.dz, gp.xmin, gp.zmin,
												gp.nsx, gp.nsz);
  ...
  if ( rValue == 1 ) {
	...
	rValue = grid.setAnisotropyAngle( theta, nTheta );
  if ( rValue == 1 ) {
		mexErrMsgTxt("Problem with anisotropy angle assignement.");
  }
