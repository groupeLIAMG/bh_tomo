/*
 * SH waves travel times with curved rays, in 2D VTI media
 *
 * ttcr2dvti_sh
 *
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs != 5) {
		mexErrMsgTxt("TTCR2DVTI_SH requires five input arguments.");
  } else if (nlhs > 2) {
  ...
  Vs0 = static_cast<double*>( mxGetPr(prhs[0]) );
  ...
  if (!(mxIsDouble(prhs[1]))) {
		mexErrMsgTxt("gamma must be double precision.");
  }
  gamma = static_cast<double*>( mxGetPr(prhs[1]) );
  number_of_dims = mxGetNumberOfDimensions(prhs[1]);
  if ( number_of_dims != 2 ) {
		mexErrMsgTxt("gamma must be a vector (nCells by 1).");
  }
  ...
  if ( stmp != 1 ) {
	...
	if ( nCells != nCells2 ) {
		mexErrMsgTxt("gamma and Vs0 must be the same size.");
  }
  ...
  Grid2Dvti_sh<double,uint32_t> grid(gp.nx, gp.nz, gp.dx, gp.dz, gp.xmin, gp.zmin,
																		 gp.nsx, gp.nsz, nThreads);
    
  if ( grid.setVs0( Vs0, nCells ) == 1 ) {
		mexErrMsgTxt("Problem with Vs0 assignement.");
  }
	  if ( grid.setGamma( gamma, nCells ) == 1 ) {
		mexErrMsgTxt("Problem with gamma assignement.");
  }
